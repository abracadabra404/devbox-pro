...
- 查询错误展示
- SQL 历史记录
- 密码保存为加密 secret 记录，连接配置只引用 `passwordRef`

验收说明：

- 已在 macOS arm64 的 Electron 应用中验证配置创建、密码引用持久化、连接测试错误展示、SQL 执行错误处理和 SQL 历史记录。
- 真实查询成功路径需要有效 MySQL 凭据；成功执行路径已通过 fake adapter 单元测试覆盖。
